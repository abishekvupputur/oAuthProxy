...
///
/// Everything is returned, not just items named "OAuthProxy …". Filtering by title is right for
...
/// OAuthProxy item someone renamed lands here, and it is the only place it is visible — but never
    ...
    bool ConfigItemPresent)
{
    ...
    public string Summary => string.Join(" ",
        new[]
        {
            IsHealthy ? $"Healthy — {OwnedItems} OAuthProxy item(s), all accounted for." : null,
            ConfigItemPresent || IsHealthy ? null : "There is no OAuthProxy Config item in this vault.",
            ...
            Others.Count > 0 ? $"{Others.Count} other item(s) in this vault, which OAuthProxy leaves alone." : null,
    ...
    ActivityLog activityLog)
{
    public async Task<VaultIntegrityReport> CheckAsync(CancellationToken ct = default)
    {
        ...
        foreach (var malformed in others.Where(i => VaultItemNaming.IsOwned(i.Title)))
        {
            orphans.Add(new VaultOrphanItem(malformed.ItemId, malformed.Title,
                "named as one of OAuthProxy's items but not in a shape it can match, so no save will touch it"));
        }
    ...
    private static string Consequence(VaultItemRole role) => role switch
    {
        VaultItemRole.Credential =>
            "its secret exists only in memory — it is lost when OAuthProxy exits unless it is written back",
        VaultItemRole.RouteKey =>
            "the route's key exists only in memory — clients keep working until OAuthProxy exits",
        VaultItemRole.FunnelKey =>
            "the funnel's key exists only in memory — clients keep working until OAuthProxy exits",
    ...
    ///
    /// Offered because the check is the only place these are visible, and an OAuthProxy item
    ...
    public async Task DeleteOtherItemAsync(VaultItemEntry item, CancellationToken ct = default)
    {
        ...
        activityLog.Log($"VAULT integrity — deleted '{item.Title}', which was not one of OAuthProxy's items");
    }
