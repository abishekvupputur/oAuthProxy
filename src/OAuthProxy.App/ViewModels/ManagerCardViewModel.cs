...
public sealed partial class ManagerCardViewModel(VaultStatus status) : ObservableObject
{
    ...
    /// <summary>
    /// The vaults this card offers: named after threeEyedRaven, and either empty or already
    /// holding an OAuthProxy configuration — the same test that decides whether picking one is
    ...
    /// <summary>Vaults that already hold an OAuthProxy configuration — one per profile.</summary>
    ...
    /// <summary>
    /// The same vaults as buttons, for the "which one should OAuthProxy use" card. Offering the
    ...
    /// <summary>
    /// The optional profile for a vault to create: blank makes threeEyedRaven, "Work" makes
    /// "threeEyedRaven Work".
    ///
    ...
    /// produced vaults that neither they nor OAuthProxy could recognise later.
    /// </summary>
    [ObservableProperty]
