...
{
    private readonly string _stubDir = Path.Combine(Path.GetTempPath(), $"oauthproxy-gate-{Guid.NewGuid()}");
    private readonly string _logPath = Path.Combine(Path.GetTempPath(), $"oauthproxy-gate-logs-{Guid.NewGuid()}");
