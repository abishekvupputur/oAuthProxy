...
{
    private readonly string _logPath = Path.Combine(Path.GetTempPath(), $"oauthproxy-sync-{Guid.NewGuid()}");
    ...
    public async Task NothingIsWrittenToDiskWhileChangesArePending()
    {
        ...
        var appData = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "OAuthProxy");
