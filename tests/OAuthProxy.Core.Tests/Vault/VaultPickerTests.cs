...
/// named after threeEyedRaven, because an app that recites the rest of someone's password manager
/// back at them has no business holding their tokens. And it is confined to vaults that would
/// actually be accepted — empty, or already OAuthProxy's — because offering one that
...
{
    private readonly string _stubDir = Path.Combine(Path.GetTempPath(), $"oauthproxy-picker-{Guid.NewGuid()}");
    private readonly string _logPath = Path.Combine(Path.GetTempPath(), $"oauthproxy-picker-logs-{Guid.NewGuid()}");
    ...
    [Fact]
    public async Task OnePassword_OnlyVaultsNamedAfterThreeEyedRavenAreOffered()
    {
    ...
    [Fact]
    public async Task ProtonPass_EmptyVaultsAndOAuthProxysOwnAreOffered()
    {
